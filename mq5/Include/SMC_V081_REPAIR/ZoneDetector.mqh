...
public:
   ...
   //--- V0.6 continuation parameters
   int    m_cont_max_bars_after_bos;
   int    m_cont_min_displacement_pts;
   int    m_cont_sl_lookback_bars;
   double m_cont_target_rr;

public:
   //--- V0.6: Init continuation parameters
   void InitContinuation(int max_bars_after_bos, int min_displacement_pts,
                         int sl_lookback_bars, double target_rr) {
      m_cont_max_bars_after_bos = max_bars_after_bos;
      m_cont_min_displacement_pts = min_displacement_pts;
      m_cont_sl_lookback_bars = sl_lookback_bars;
      m_cont_target_rr = target_rr;
   }

   //--- V0.6: Check if continuation entry is viable after BOS
   //    Does NOT require retrace to OB
   bool EvaluateContinuation(ENUM_TIMEFRAMES tf,
                              double swing_high_price, datetime swing_high_time,
                              double swing_low_price,  datetime swing_low_time,
                              StructConfirmationState &conf,
      ...
      cont_status = CONTINUATION_NONE;

      // Phase 1: Sweep + BOS + OB (same as retrace path)
      StructZoneState zone;
      EvaluateZones(tf, swing_high_price, swing_high_time,
                    swing_low_price, swing_low_time, zone);
      ...
      // Check if BOS is recent enough (within max_bars_after_bos)
      int bos_shift = iBarShift(_Symbol, tf, zone.bos_after_sweep.time);
      if(bos_shift < 0 || bos_shift > m_cont_max_bars_after_bos) {
         return false; // BOS too old for continuation
      }

      // Check displacement: price must have moved min_displacement_pts from OB
      double current_price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      double displacement = 0;
      if(ob_type == OB_BULLISH) {
         displacement = current_price - zone.last_ob.ob_high;
      } else {
         displacement = zone.last_ob.ob_low - current_price;
      }

      double displacement_pts = displacement / _Point;
      if(displacement_pts < m_cont_min_displacement_pts) {
         return false; // Not enough displacement
      }

      // Continuation candidate found!
      entry_mode  = ENTRY_MODE_CONTINUATION;
      cont_status = CONTINUATION_CANDIDATE;

      // V0.6: M5 continuation confirmation
      // Check M5 close breaks minor swing in trend direction
      int retrace_shift = iBarShift(_Symbol, PERIOD_M5, zone.bos_after_sweep.time);
      if(retrace_shift < 0) retrace_shift = 10;

      int search_end = retrace_shift + m_cont_sl_lookback_bars;
      if(search_end > 200) search_end = 200;

      double mss_price = 0;
      datetime mss_time = 0;
      bool m5_confirmed = false;

      if(ob_type == OB_BULLISH) {
         if(m_ms.DetectBullishMSS_M5(1, search_end, zone.bos_after_sweep.time, mss_price, mss_time)) {
            // Also check for a bullish candle
            int m5_shift = iBarShift(_Symbol, PERIOD_M5, mss_time);
            if(m5_shift >= 1) {
         ...
         if(m_ms.DetectBearishMSS_M5(1, search_end, zone.bos_after_sweep.time, mss_price, mss_time)) {
            int m5_shift = iBarShift(_Symbol, PERIOD_M5, mss_time);
            if(m5_shift >= 1) {
               double open_m5 = iOpen(_Symbol, PERIOD_M5, 1);
               double close_m5 = iClose(_Symbol, PERIOD_M5, 1);
      ...
      if(!m5_confirmed) {
         cont_status = CONTINUATION_CANDIDATE;
         conf.final_status = ZONE_WAIT_CONFIRMATION;
         return true; // Candidate exists but not yet confirmed
      }

      cont_status = CONTINUATION_CONFIRMED;
      if(ob_type == OB_BULLISH) {
         conf.final_status = ZONE_READY_BUY;
      } else {
         conf.final_status = ZONE_READY_SELL;
