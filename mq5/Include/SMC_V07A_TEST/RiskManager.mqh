...
public:
   ...
   //--- V0.7A: Apply fixed minimum SL for continuation entries
   //    If swing-based SL is tighter than fixed_min, widen it
   void ApplyFixedMinSL(StructTradePlan &plan, int fixed_min_sl_pts, double target_rr) {
      if(plan.plan_status != TRADE_PLAN_VALID) return;
      if(plan.sl_points >= fixed_min_sl_pts) return; // Already wide enough
      if(plan.entry_price <= 0) return;
      
      UpdateSymbolInfo();
      
      double new_sl_price;
      double new_sl_pts = fixed_min_sl_pts;
      
      // Widen SL to fixed minimum
      if(plan.direction == TRADE_DIR_BUY) {
         new_sl_price = plan.entry_price - PointsToPrice(new_sl_pts);
      } else {
         new_sl_price = plan.entry_price + PointsToPrice(new_sl_pts);
      }
      
      // New risk distance
      double risk_dist = fixed_min_sl_pts * m_point;
      
      // New TP based on target RR
      double tp_distance = risk_dist * target_rr;
      double new_tp_price;
      if(plan.direction == TRADE_DIR_BUY) {
         new_tp_price = plan.entry_price + tp_distance;
      } else {
         new_tp_price = plan.entry_price - tp_distance;
      }
      
      // Update plan
      plan.sl_price = new_sl_price;
      plan.sl_points = new_sl_pts;
      plan.tp_price = new_tp_price;
      plan.tp_points = tp_distance / m_point;
      plan.rr = target_rr; // May differ from original
      
      // Recalculate lot with wider SL
      // Risk money is already calculated
      string lot_error = "";
      double new_lot = CalculateLotSize(plan.risk_money, new_sl_pts, lot_error);
      
      // Log if lot becomes too small
      if(new_lot <= 0 && plan.lot_size > 0) {
         Print("V0.7A WARNING: Fixed SL ", fixed_min_sl_pts, " pts reduces lot below min. "
               "Original lot: ", plan.lot_size, " Balance may be too small for wider SL.");
      }
      
      if(new_lot > 0) {
         plan.lot_size = new_lot;
      }
      // If new_lot = 0, keep original plan (will be rejected by lot check)
   }
