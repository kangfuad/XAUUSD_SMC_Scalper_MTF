...
public:
   ...
   //--- V0.6.1: Trend to String (public wrapper)
   string GetTrendString(ENUM_TREND_BIAS trend) {
      return TrendToString(trend);
   }
   ...
   //--- Close current CSV if open (for switching between CSV files)
   void CloseCSVIfOpen() {
      if(m_csv_opened && m_csv_handle != INVALID_HANDLE) {
         FileClose(m_csv_handle);
         m_csv_handle = INVALID_HANDLE;
         m_csv_opened = false;
      }
   ...
   //--- V0.6: Entry Mode to String
   string EntryModeToString(ENUM_ENTRY_MODE mode) {
      if(mode == ENTRY_MODE_RETRACE)      return "RETRACE";
      if(mode == ENTRY_MODE_CONTINUATION) return "CONTINUATION";
      return "NONE";
   }
   
   //--- V0.6: Continuation Status to String
   string ContinuationStatusToString(ENUM_CONTINUATION_STATUS status) {
      if(status == CONTINUATION_CANDIDATE)  return "CANDIDATE";
      if(status == CONTINUATION_CONFIRMED)  return "CONFIRMED";
      return "NONE";
   }
   
   //--- V0.6: Open V0.6 Continuation CSV
   bool OpenCSVV06Continuation() {
      CloseCSVIfOpen();
      string filename = "XAUUSD_SMC_V06_ContinuationLog.csv";
      bool fe = FileIsExist(filename);
      m_csv_handle = FileOpen(filename, FILE_WRITE|FILE_READ|FILE_CSV|FILE_ANSI, ',');
      if(m_csv_handle == INVALID_HANDLE) return false;
      if(!fe || FileSize(m_csv_handle) == 0) {
         FileSeek(m_csv_handle, 0, SEEK_END);
         FileWrite(m_csv_handle,
            "time","symbol","m15_trend",
            "sweep_type","sweep_time","bos_type","bos_time",
            "ob_type","ob_time",
            "entry_mode","continuation_status",
            "m5_confirm_type","m5_confirm_time",
            "candle_confirm","ready_status","reason");
         FileFlush(m_csv_handle);
      } else FileSeek(m_csv_handle, 0, SEEK_END);
      m_csv_opened = true;
      return true;
   }
   
   //--- V0.6: Log Continuation State (Print)
   void LogContinuationPrint(const string symbol, const string signal_status,
                              ENUM_ENTRY_MODE entry_mode,
                              ENUM_CONTINUATION_STATUS cont_status,
                              const string detail) {
      string mode_str = EntryModeToString(entry_mode);
      string cont_str = ContinuationStatusToString(cont_status);
      string time_str = TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS);
      string log = StringFormat("[%s] %s | %s | Mode:%s Cont:%s | %s",
                                time_str, symbol, signal_status, mode_str, cont_str, detail);
      Print(log);
   }
   
   //--- V0.6: Log Continuation CSV
   void LogContinuationCSV(const string symbol,
                            const string m15_trend_str,
                            const StructConfirmationState &conf,
                            ENUM_ENTRY_MODE entry_mode,
                            ENUM_CONTINUATION_STATUS cont_status,
                            const string sweep_str,
                            const string bos_str,
                            const string ob_str,
                            const string reason) {
      if(!OpenCSVV06Continuation()) return;
      string ts = TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS);
      FileWrite(m_csv_handle, ts, symbol, m15_trend_str,
         sweep_str, "", bos_str, "",
         ob_str, "",
         EntryModeToString(entry_mode), ContinuationStatusToString(cont_status),
         M5ConfirmTypeToString(conf.m5_confirm.type),
         (conf.m5_confirm.is_valid ? TimeToString(conf.m5_confirm.time, TIME_DATE|TIME_SECONDS) : "NONE"),
         CandleConfirmTypeToString(conf.candle_confirm.type),
         ZoneStatusToString(conf.final_status),
         reason);
      FileFlush(m_csv_handle);
   }
   
   //--- V0.6: Open V0.6 Execution CSV
   bool OpenCSVV06Execution() {
      CloseCSVIfOpen();
      string filename = "XAUUSD_SMC_V06_ExecutionLog.csv";
      bool fe = FileIsExist(filename);
      m_csv_handle = FileOpen(filename, FILE_WRITE|FILE_READ|FILE_CSV|FILE_ANSI, ',');
      if(m_csv_handle == INVALID_HANDLE) return false;
      if(!fe || FileSize(m_csv_handle) == 0) {
         FileSeek(m_csv_handle, 0, SEEK_END);
         FileWrite(m_csv_handle,
            "time","symbol","entry_mode","signal_status","signal_time",
   ...
   //--- V0.6: Log Execution CSV to V06 file
   void LogExecutionCSVV06(const string symbol,
                           const string mode,
                           const StructTradePlan &plan,
                           ENUM_EXECUTION_STATUS exec_status,
                           ulong ticket,
                           ...
                           const string reason) {
      if(exec_status == EXECUTION_DISABLED) return;
      if(!OpenCSVV06Execution()) return;
      
      string ts   = TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS);
      string sig  = TimeToString(plan.signal_time, TIME_DATE|TIME_SECONDS);
      string dir  = TradeDirectionToString(plan.direction);
      string stat = ExecutionStatusToString(exec_status);
      string tk   = (ticket > 0) ? IntegerToString(ticket) : "0";
      string rc   = (retcode > 0) ? IntegerToString(retcode) : "0";
      string ec   = (error_code > 0) ? IntegerToString(error_code) : "0";
      string em   = EntryModeToString(plan.entry_mode);
      
      string es   = (plan.entry_price > 0) ? DoubleToString(plan.entry_price, 5) : "0";
      string ss   = (plan.sl_price > 0) ? DoubleToString(plan.sl_price, 5) : "0";
      string ts2  = (plan.tp_price > 0) ? DoubleToString(plan.tp_price, 5) : "0";
      string ls   = (plan.lot_size > 0) ? DoubleToString(plan.lot_size, 2) : "0";
      string rrs  = (plan.rr > 0) ? DoubleToString(plan.rr, 2) : "0";
      string rms  = (plan.risk_money > 0) ? DoubleToString(plan.risk_money, 2) : "0";
      string sps  = (plan.spread_points > 0) ? DoubleToString(plan.spread_points, 0) : "0";
      ...
      FileWrite(m_csv_handle,
         ts, symbol, em,
         (plan.direction == TRADE_DIR_BUY ? "READY_BUY" : "READY_SELL"), sig,
         dir, es, ss, ts2,
         ls, rrs, rms, sps,
         IntegerToString(plan.stop_level_points),
         stat, tk, rc, ec,
   ...
   //--- V0.7A.1: Second CSV handle for V07A execution log
   int    m_csv_v07a;
   bool   m_csv_v07a_open;
   
   //--- V0.7A.1: Open V07A CSV
   bool OpenCSVV07A() {
      if(m_csv_v07a_open) return true;
      string filename = "XAUUSD_SMC_V07A_ExecutionLog.csv";
      bool fe = FileIsExist(filename);
      m_csv_v07a = FileOpen(filename, FILE_WRITE|FILE_READ|FILE_CSV|FILE_ANSI, ',');
      if(m_csv_v07a == INVALID_HANDLE) {
         Print("CSV ERROR: Cannot open V07A CSV. Error: ", GetLastError());
         return false;
      }
      if(!fe || FileSize(m_csv_v07a) == 0) {
         FileSeek(m_csv_v07a, 0, SEEK_END);
         FileWrite(m_csv_v07a,
            "time","test_name","symbol","deposit","fixed_sl_pts","fixed_tp_pts",
            "direction","entry_time","exit_time","entry","sl","tp",
            "sl_pts","tp_pts","rr","lot","risk_money","spread_pts",
            "order_ticket","deal_ticket","magic","entry_mode",
            "status","exit_reason","profit","balance","equity","dd_pct","reason");
         FileFlush(m_csv_v07a);
      } else FileSeek(m_csv_v07a, 0, SEEK_END);
      m_csv_v07a_open = true;
      return true;
   }
   
   //--- V0.7A.1: Log order sent to V07A CSV
   void LogOrderSentV07A(const string test_name,
                          const string symbol,
                          double deposit,
                          int fixed_sl_pts, int fixed_tp_pts,
                          const StructTradePlan &plan,
                          ulong ticket, uint retcode,
                          ENUM_EXECUTION_STATUS status) {
      if(!OpenCSVV07A()) return;
      
      string ts = TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS);
      string dir = TradeDirectionToString(plan.direction);
      string sig_time = (plan.signal_time > 0) ? TimeToString(plan.signal_time, TIME_DATE|TIME_SECONDS) : ts;
      
      FileWrite(m_csv_v07a,
         ts, test_name, symbol,
         DoubleToString(deposit, 0),
         IntegerToString(fixed_sl_pts), IntegerToString(fixed_tp_pts),
         dir, sig_time, "0",
         DoubleToString(plan.entry_price, 5),
         DoubleToString(plan.sl_price, 5),
         DoubleToString(plan.tp_price, 5),
         DoubleToString(plan.sl_points, 0),
         DoubleToString(plan.tp_points, 0),
         DoubleToString(plan.rr, 2),
         DoubleToString(plan.lot_size, 2),
         DoubleToString(plan.risk_money, 2),
         DoubleToString(plan.spread_points, 0),
         IntegerToString((int)ticket), "0",
         IntegerToString(plan.stop_level_points),
   ...
   //--- V0.7A.1: Close V07A CSV handle
   void CloseCSVV07A() {
      if(m_csv_v07a != INVALID_HANDLE && m_csv_v07a_open) {
         FileClose(m_csv_v07a);
         m_csv_v07a = INVALID_HANDLE;
         m_csv_v07a_open = false;
      }
   }
};
